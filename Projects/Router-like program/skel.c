...
void parse_route_table(RouteTableEntry *routeTable, unsigned int *adrRouteTableSize, char *argv[])
{
	*adrRouteTableSize = 0;

	FILE *input = fopen(argv[1], "r");
	DIE(input == NULL, "File opening error");

	char prefixStr[16], nextHopStr[16], maskStr[16];
	int interface;

	while ((fscanf(input, "%s %s %s %d", prefixStr, nextHopStr, maskStr, &interface) > 0))
	{

		routeTable[*adrRouteTableSize].prefix = inet_addr(prefixStr);
		routeTable[*adrRouteTableSize].nextHop = inet_addr(nextHopStr);
		routeTable[*adrRouteTableSize].mask = inet_addr(maskStr);
		routeTable[(*adrRouteTableSize)++].interface = interface;

		memset(prefixStr, 0x00, sizeof(prefixStr));
		memset(nextHopStr, 0x00, sizeof(prefixStr));
		memset(maskStr, 0x00, sizeof(prefixStr));
	}

	return;
}

RouteTableEntry *get_best_route(uint32_t destination, RouteTableEntry *routeTable, int routeTableSize)
{
	uint32_t bestMask = 0;
	int resultEntryIndex = -1;
	...
	while (i < routeTableSize)
	{
		...
		if ((destination & entry.mask) == entry.prefix)
		{
			if (bestMask < entry.mask)
			{
				bestMask = entry.mask;
				resultEntryIndex = i;
			}
	...
	return &routeTable[resultEntryIndex];
}

ArpTableEntry *get_arp_entry(__u32 ip, ArpTableEntry *arpTable, int arpTableSize)
{
